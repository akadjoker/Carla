...
def average_slope_intercept(image, lines):
    ...
    for line in lines:
        for x1, y1, x2, y2 in line:
            ...
            slope = fit[0]
            intercept = fit[1]
            if slope: # y is reversed in image
...
def reproduzir_video_com_perspectiva(caminho_video):
    ...
    while True:
        ...
        canny = cv2.Canny(result_masked, 50, 150)
        lines = cv2.HoughLinesP(canny, 2, np.pi/180, 100, np.array([]), minLineLength=40,maxLineGap=5)
        lines_image = display_lines(result_masked, lines)
        combo_image = cv2.addWeighted(result_masked, 0.8, lines_image, 1, 0)
